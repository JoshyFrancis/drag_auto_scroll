...
//                    this.cur_e.clientY = this.starte.clientY + this.ky;
        ...
        function resize_auto_scroll(el_r) {
            var span = document.createElement('span');
            span.style.position = 'absolute';//must have
            span.style.right = '0px';//must have
            span.style.bottom = '0px'; //must have
            span.style.cursor = 'se-resize';
//            span.style['z-index'] = '10000';
//            span.style.backgroundColor = 'red';
            el_r.style.overflow = 'hidden';

//            span.innerHTML = '<img src="resize_handle2.gif" width=16 height=16 />';
            span.innerHTML = '<img width=16 height=16 src="data:image/gif;base64,R0lGODlhGAAYAKEBAIiIiP///////////yH5BAEKAAIALAAAAAAYABgAAAJAlI+pFusbwJtM0hcvtlpl1h2fMGpjeZ2cyaGUKqbt/NJVLdpbzk/wDfLofggisUcaroBJpMs4ZEJ5U6mk2gQWAAA7" />';
            el_r.appendChild(span);

            var rd1 = new drag_auto_scroll(span);
            rd1.restricttoscrollview = false;
            rd1.elr = el_r;
            rd1.elspan = span;
            
            var rh1 = document.createElement('div');
//            rh1.innerHTML = 'Drag me';
//            rh1.style.backgroundColor = 'green';
            rh1.style.border = '0px solid red';
            rh1.style.position = 'absolute';
            rh1.style.cursor = 'se-resize';
            rh1.style.width = '50px';
            rh1.style.height = '50px';
            rh1.style.display = 'none';
            rd1.elr.parentNode.appendChild(rh1);
            ...
            rd1.ghost = null;
			rd1.mousedown=function(e){
				this.move(e);
			};
            rd1.beforedrag = function (dx, dy, mx, my, e) {
				var offset = this.getposition(this.elr );
				//info.innerHTML='<br>'+JSON.stringify(offset);
                var scrollLeft = this.scrollLeft(this.elr.parentNode);
                var scrollTop = this.scrollTop(this.elr.parentNode);
                this.w =this.sw = offset.width;
                this.h =this.sh = offset.height;
				this.sx =dx; 
                this.sy =dy; 
                if (this.ghost) {
                    this.ghost.innerHTML = this.elr.innerHTML;
                    this.ghost.style.overflow = 'hidden';
                    this.ghost.style.display = '';
                    this.fade(this.ghost, 35);
						if (this.ghost.parentNode != document.body) {
							var poffset = this.ghost.parentNode.getBoundingClientRect();
                    ...
                    this.ghost.style.width = offset.width + 'px';
                    this.ghost.style.height = offset.height + 'px';
                }
                if (this.resizestart) {
            ...
            rd1.dragover = function (dropinfo, el, dx, dy, mx, my, e) {
				var w = (this.sw + dx  -this.sx );
                var h = (this.sh +dy   -this.sy );
                w = Math.floor(w / this.xcell) * this.xcell;
                h = Math.floor(h / this.ycell) * this.ycell;
                //info.innerHTML='<br>'+ (this.sx  )+'x'+(this.sy  );
                //info.innerHTML+='<br>'+ (e.clientX+scrollLeft  )+'x'+(e.clientY+ scrollTop  );
                //info.innerHTML+='<br>'+ (w  )+'x'+(h  );
                this.w = w;
                this.h = h;
                if (w > 0 && h > 0) {
                    if (this.ghost) {
                        ...
                        this.elr.style.width = w + 'px';
                        this.elr.style.height = h + 'px';
                    }
                }
                if (this.resize) {
                    this.resize();
            ...
            rd1.dragdrop = function (dropinfo, el, dx, dy, e) {
                if (this.ghost) {
                    this.ghost.style.display = 'none';
                    if (this.w > 0 && this.h > 0) {
                        this.elr.style.width = this.w + 'px';
                        this.elr.style.height = this.h + 'px';
                    }
                }
                if (this.resizestop) {
                    this.resizestop();
